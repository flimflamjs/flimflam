...
function init(state) {
  ...
  state.val$ = flyd.mergeAll([flyd.map(ev => ev.currentTarget.value, state.keyup$), state.select$]);
  // Stream of events when the user starts on the input and it's empty (ie focus on empty input, or keyup$ to empty value)
  const empty$ = flyd.map(() => state.values$(), flyd.filter(ev => !ev.currentTarget.value, flyd.merge(state.focus$, state.keyup$)));

  const withValues$ = flyd.lift(R.pair, state.val$, state.values$);
  state.partialMatches$ = flyd.scanMerge([[withValues$, filterValues], [empty$, (matches, vals) => state.showInitial ? vals : []], [state.select$, R.always([])], [state.blur$, R.always([])]], []);
  ...
  state.dropdownIdx$ = flyd.scanMerge([[state.upArrow$, idx => idx === 0 ? 0 : idx - 1] // decrement, but not below 0
  , [downWithLength$, (idx, matches) => idx >= matches.length - 1 ? matches.length - 1 : idx + 1]], 0);
...
function view(vnode, state) {
  ...
  let dropdown = h('div.ff-autocomplete-dropdown', matches.length ? R.addIndex(R.map)(dropdownValue(state), matches) : '');
